...
/***/ function(module, exports, __webpack_require__) {
	...
	var onUpdateGame = function onUpdateGame() {
	  api.updateGame(gameState.getData()).done(ui.updateGameSuccess).fail(ui.updateGameFailure);
	};

	var onGetGames = function onGetGames(event) {
	  event.preventDefault();
	  api.getGames().then(ui.getGameSuccess).catch(ui.failure);
	  ui.displayStats();
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getGameSuccess = function getGameSuccess(data) {
	  ...
	  $('#completed-games').text('Games Completed: ' + logic.calcCompletedGames(data));
	};
